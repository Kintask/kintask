...
interface ChatInterfaceProps {
  ...
  // Signature matches the updated apiService call in App.tsx
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, onSendMessage, isLoading }) => {
  ...
  // State specifically for the knowledge base CID
  const [knowledgeBaseCid, setKnowledgeBaseCid] = useState('');
  ...
  const handleKnowledgeBaseCidChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setKnowledgeBaseCid(event.target.value);
  };
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    // Pass the knowledgeBaseCid state to the handler
    onSendMessage(trimmedInput, trimmedKnowledgeBaseCid || undefined);
    setInputValue('');
    setKnowledgeBaseCid(''); // Clear knowledge base CID input after sending
  };
      ...
      <div className="shrink-0 border-t border-gray-200 dark:border-gray-700 p-4 bg-gray-50/50 dark:bg-gray-800/50 backdrop-blur-sm">
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
           ...
           <div className="flex items-center space-x-3">
            ...
            {/* Optional Knowledge Base CID Input Row */}
            <div className="mt-2">
                 <input
                   type="text"
                   value={knowledgeBaseCid} // Use dedicated state
                   onChange={handleKnowledgeBaseCidChange} // Use dedicated handler
                   placeholder="Optional Knowledge Base CID (Qm...)" // Updated placeholder
                   className="w-full px-4 py-2 text-sm bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full focus:outline-none focus:ring-1 focus:ring-kintask-blue dark:focus:ring-kintask-blue-light text-gray-700 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 disabled:opacity-60"
                   disabled={isLoading}
